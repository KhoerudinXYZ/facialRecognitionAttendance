...
class AbsensiSmokeTest extends TestCase
{
    ...
    public function test_kelas_yang_masih_ada_siswa_tidak_bisa_dihapus(): void
    {
        $this->actingAs($this->admin());
        $kelas = $this->kelas();
        $siswa = Siswa::create([
            'nis' => '555', 'nama' => 'Budi', 'jenis_kelamin' => 'L', 'kelas_id' => $kelas->id,
        ]);
        Absensi::create([
            'siswa_id' => $siswa->id, 'kelas_id' => $kelas->id, 'tanggal' => '2026-07-10',
            'jam_masuk' => '07:00:00', 'status' => 'hadir', 'metode' => 'face',
        ]);

        $this->delete("/kelas/{$kelas->id}")->assertRedirect()->assertSessionHas('error');

        // Kelas, siswa, DAN seluruh riwayat absensinya harus tetap utuh --
        // kelas_id.cascadeOnDelete di siswa (lalu siswa_id.cascadeOnDelete
        // di absensi/pengajuan_izin/face_descriptors) bikin hapus satu
        // Kelas diam-diam menghapus semua riwayat murid di dalamnya kalau
        // tidak diblokir di sini duluan.
        $this->assertDatabaseHas('kelas', ['id' => $kelas->id]);
        $this->assertDatabaseHas('siswa', ['id' => $siswa->id]);
        $this->assertDatabaseHas('absensi', ['siswa_id' => $siswa->id]);
    }

    public function test_kelas_kosong_bisa_dihapus(): void
    {
        $this->actingAs($this->admin());
        $kelas = $this->kelas();

        $this->delete("/kelas/{$kelas->id}")->assertRedirect()->assertSessionHas('success');
    ...
    public function test_absensi_manual_koreksi_hadir_ke_sakit_menghapus_jam_pulang_lama(): void
    {
        $this->actingAs($this->admin());
        $kelas = $this->kelas();
        $siswa = Siswa::create([
            'nis' => '333', 'nama' => 'Rina', 'jenis_kelamin' => 'P', 'kelas_id' => $kelas->id,
        ]);
        ...
        $this->post('/absensi/manual', [
            ...
            'tanggal' => '2026-07-13',
            'status' => 'sakit',
            'keterangan' => 'Surat dokter menyusul',
        ])->assertRedirect();
    ...
    public function test_absensi_manual_koreksi_terlambat_ke_hadir_tetap_pertahankan_jam_pulang(): void
    {
        $this->actingAs($this->admin());
        $kelas = $this->kelas();
        $siswa = Siswa::create([
            'nis' => '444', 'nama' => 'Doni', 'jenis_kelamin' => 'L', 'kelas_id' => $kelas->id,
        ]);
        ...
        $this->post('/absensi/manual', [
            ...
            'tanggal' => '2026-07-13',
            'status' => 'hadir',
            'keterangan' => 'Koreksi, sebenarnya tepat waktu',
        ])->assertRedirect();
    ...
    public function test_race_input_manual_dengan_scan_wajah_konkuren_tetap_tersimpan(): void
    {
        $this->actingAs($this->admin());
        $kelas = $this->kelas();
        $siswa = Siswa::create([
            'nis' => '666', 'nama' => 'Eka', 'jenis_kelamin' => 'P', 'kelas_id' => $kelas->id,
        ]);

        // Simulasikan AbsensiRecorder (scan wajah siswa yang sama) menang
        // race TEPAT di antara pengecekan first() milik manual() dan
        // INSERT-nya sendiri.
        ...
        // Koreksi admin TETAP harus tersimpan (bukan 500), dan hasil
        // akhirnya status sakit, bukan baris hadir pemenang race yang
