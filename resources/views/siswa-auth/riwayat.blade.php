                ...
                <div class="space-y-3">
                    @foreach ($riwayatGabungan as $item)
                        ...
                        <div class="{{ $rowClass }}">
                            ...
                            <div class="shrink-0 flex flex-row sm:flex-col items-center sm:items-end justify-between sm:justify-center gap-2.5 pt-2 sm:pt-0 border-t sm:border-t-0 border-slate-100 dark:border-slate-800/80 w-full sm:w-auto">
                                ...
                                @if (! $isLibur)
                                    ...
                                    @if ($koreksi && in_array($koreksi->status, ['menunggu', 'disetujui'], true))
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[10px] font-black font-lexend uppercase tracking-wider {{ $koreksi->status === 'menunggu' ? 'text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200/80 dark:border-amber-800/60' : 'text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-800/60' }}">
                                            <x-icon name="alert-circle" class="w-3.5 h-3.5 stroke-[2.5] shrink-0" />
                                            <span>Koreksi {{ $koreksi->status === 'menunggu' ? 'Menunggu' : 'Disetujui' }}</span>
                                        </span>
                                    @else
                                        <x-confirm-form :action="route('siswa.koreksi.store')" method="POST" enctype="multipart/form-data"
                                                         ...
                                                         trigger-class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-[10px] font-black font-lexend uppercase tracking-wider text-slate-500 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-300 bg-slate-100/80 dark:bg-slate-800/60 hover:bg-indigo-50 dark:hover:bg-indigo-950/50 border border-slate-200/80 dark:border-slate-700/80 shadow-sm transition-all duration-200 cursor-pointer active:scale-95 group">
                                            <x-slot:fields>
                                            ...
                                            <x-icon name="alert-circle" class="w-3.5 h-3.5 stroke-[2.5] shrink-0 text-amber-500 dark:text-amber-400 group-hover:scale-110 transition-transform" />
