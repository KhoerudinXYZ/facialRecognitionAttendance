<x-app-layout>
    ...
    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        ...
        @forelse ($pengajuanList as $p)
            <div class="bento-card rounded-[2rem] p-6 sm:p-7 shadow-xl relative overflow-hidden group transition-all duration-300 hover:scale-[1.005] {{ $p->status === 'menunggu' ? 'border-amber-200/50 dark:border-amber-800/30' : '' }}">
                ...
                @endif
                <div class="absolute -right-4 -bottom-4 text-[50px] font-black text-slate-900/[0.025] dark:text-white/[0.015] font-lexend pointer-events-none tracking-tighter leading-none select-none uppercase">{{ $p->jenis }}</div>

                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-5 relative z-10">
                    {{-- Left: Student Info --}}
                    <div class="flex items-start gap-4 flex-1 min-w-0">
                        <div class="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 text-white font-black font-lexend flex items-center justify-center shadow-lg shadow-indigo-500/30 shrink-0 text-lg">
                            {{ Illuminate\Support\Str::of($p->siswa->nama)->substr(0, 1)->upper() }}
                        </div>
                    ...
                    <div class="flex items-center gap-2 shrink-0">
                        @if ($p->status === 'menunggu')
                            <x-confirm-form :action="route('pengajuan-izin.approve', $p)" method="POST"
                                ...
                                <x-icon name="check-circle" class="w-4 h-4 stroke-[2.5]" /> Approve
                            </x-confirm-form>
                            <x-confirm-form :action="route('pengajuan-izin.reject', $p)" method="POST"
                                             title="Tolak pengajuan {{ $p->siswa->nama }}?"
                                             confirm-label="Tolak"
                                             trigger-class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-300 font-black font-lexend text-[10px] uppercase tracking-wider transition-all duration-300 border border-slate-200/50 dark:border-slate-700/50">
                                <x-slot:fields>
                                    <div>
                                ...
                                <x-icon name="x-circle" class="w-4 h-4 stroke-[2.5]" /> Reject
                            </x-confirm-form>
                        @endif
                    </div>
                </div>
