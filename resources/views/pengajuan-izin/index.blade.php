<x-app-layout>
    ...
    <div class="py-8 max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-4"
         x-data="{ rejectOpen: false, rejectAction: '', pengajuanNama: '' }">
                            ...
                            <td class="px-6 py-3 text-right space-x-2 whitespace-nowrap">
                                @if ($p->status === 'menunggu')
                                    ...
                                    <x-secondary-button type="button"
                                        @click="rejectOpen = true; rejectAction = '{{ route('pengajuan-izin.reject', $p) }}'; pengajuanNama = '{{ addslashes($p->siswa->nama) }}'">
                                        Reject
                                    </x-secondary-button>
                                @endif
                            </td>
        ...
        {{-- Modal reject --}}
        <div x-show="rejectOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="rejectOpen = false">
            <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-md p-6">
                <h3 class="font-semibold text-gray-800 dark:text-gray-100 mb-4">Tolak Pengajuan — <span x-text="pengajuanNama"></span></h3>
                <form :action="rejectAction" method="POST" class="space-y-4">
                    @csrf
                    <div>
                        <x-input-label for="catatan_admin" value="Catatan (opsional)" />
                        <x-text-input id="catatan_admin" name="catatan_admin" type="text" class="mt-1 block w-full" />
                    </div>
                    <div class="flex justify-end gap-3">
                        <button type="button" @click="rejectOpen = false" class="px-4 py-2 text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200">Batal</button>
                        <x-primary-button>Tolak</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
