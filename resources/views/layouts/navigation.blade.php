<nav x-data="{ open: false }" class="bg-white/50 dark:bg-slate-900/80 backdrop-blur-xl backdrop-saturate-150 border-b border-indigo-100/60 dark:border-slate-800/50 sticky top-0 z-40 transition-colors duration-300 shadow-[0_1px_3px_rgba(99,102,241,0.04)]">
                    ...
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 dark:text-gray-300 bg-white dark:bg-gray-800 hover:text-gray-700 dark:hover:text-white focus:outline-none transition ease-in-out duration-150">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-gradient-to-r from-indigo-500 to-purple-500 text-white">
                                    {{ Auth::user()->isAdmin() ? 'Administrator' : (Auth::user()->isWaliKelas() ? 'Wali Kelas' : '') }}
                                </span>
                            </div>
