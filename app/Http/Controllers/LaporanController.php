...
class LaporanController extends Controller
{
    public function index(Request $request)
    {
        ...
        $liburDalamPeriode = $this->hitungLiburDalamPeriode($dari, $sampai);
        ...
        $barisTanpaWali = $data->filter(fn (Absensi $a) => ! $a->kelas?->waliKelas)->count();
    ...
    {
        if ($kelasId) {
            return Kelas::visibleTo($request->user())->find($kelasId);
        }
    ...
    /**
     * Hitung hari libur dalam rentang laporan, gabungan hari libur manual
     * (tabel hari_libur) DAN hari libur mingguan rutin dari Pengaturan --
     * konsisten dengan HariLibur::isLibur() yang dipakai di seluruh alur
     * absensi. Query HariLibur::whereBetween(...)->count() saja tidak
     * cukup: itu cuma menghitung baris manual, jadi rentang dengan libur
     * mingguan aktif (mis. Sabtu) akan under-count hari libur di laporan.
     */
    private function hitungLiburDalamPeriode(Carbon $dari, Carbon $sampai): int
    {
        $liburMingguan = Pengaturan::get()->liburMingguan();

        $tanggalLiburManual = HariLibur::whereBetween('tanggal', [$dari->toDateString(), $sampai->toDateString()])
            ->pluck('tanggal')
            ->map(fn ($tanggal) => Carbon::parse($tanggal)->toDateString())
            ->all();

        $jumlah = 0;
        $cursor = $dari->copy()->startOfDay();
        $akhir = $sampai->copy()->startOfDay();

        while ($cursor->lte($akhir)) {
            if (in_array($cursor->dayOfWeek, $liburMingguan, true) || in_array($cursor->toDateString(), $tanggalLiburManual, true)) {
                $jumlah++;
            }
    ...
    private function queryLaporan(Request $request, Carbon $dari, Carbon $sampai, ?int $kelasId)
    {
        ...
        if ($kelasId) {
            // Filter kelas EKSPLISIT: otorisasi berdasarkan kelas ITU SENDIRI
            // (Kelas::visibleTo -- apakah user berhak lihat kelas ini
            // sekarang), baru ambil barisnya lewat snapshot kelas_id (lihat
            // migration add_kelas_id_to_absensi & Absensi::kelas()).
            // SENGAJA TIDAK di-AND dengan Absensi::visibleTo() yang
            // berbasis kelas siswa SEKARANG -- kalau di-AND, baris riwayat
            // siswa yang sudah pindah KELUAR dari kelas ini hilang dari
            // laporan kelas asalnya sendiri, padahal wali kelas lama justru
            // berhak & butuh lihat laporan kelas binaannya sendiri secara
            // historis. Sebagai bonus, ini juga menutup kebocoran sebaliknya:
            // wali kelas tidak lagi bisa "mengintip" baris dari kelas yang
            // TIDAK PERNAH dia bina cuma karena siswanya kebetulan sekarang
            // ada di kelas binaannya.
            if (! Kelas::visibleTo($request->user())->whereKey($kelasId)->exists()) {
                return $query->whereNull('id');
            }

            $query->where('kelas_id', $kelasId);
        } else {
            // Tanpa filter kelas spesifik: "kelas mana saja yang saya bina
            // SEKARANG" tetap berbasis kelas siswa SEKARANG (current roster)
            // -- lihat docblock Absensi::scopeVisibleTo().
